...
@Component({
    ...
    template: `
        <div class="paginator">
            ...
            <button type="button" [disabled]="currentPage === 1" (click)="changePageToPrev($event)"
                    class="paginator-prev">
                <span> < </span>
            </button>
            <span class="paginator-pages">
                <button type="button" *ngFor="let pageLink of pageLinks" class="paginator-page"
                        [class.highlight]="pageLink == currentPage"
                        (click)="onPageLinkClick($event, pageLink)">{{pageLink}}</button>
            </span>
            <button type="button" [disabled]="currentPage === lastPage" (click)="changePageToNext($event)"
            ...
            <select [(ngModel)]="rows" class="paginator-rows-per-page" (ngModelChange)="onRppChange()">
                <option *ngFor="let item of rowsPerPageOptions" [value]="item">{{item}}</option>
            </select>
        </div>
    `,
